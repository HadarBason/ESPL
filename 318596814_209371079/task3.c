...
void PrintSymbols(state * s){
    ...
    Elf32_Shdr * symbolNames;
    int size;
    for(int i=0; i < map_file.header -> e_shnum; i++){
        if(map_file.section_header[i].sh_type == SHT_SYMTAB){
            symbolTable = (Elf32_Sym*)( map_file.mapPtr + map_file.section_header[i].sh_offset);//calculation
            size = map_file.section_header[i].sh_size / sizeof(Elf32_Sym);//size of table
            symbolNames = map_file.mapPtr + (&map_file.section_header[map_file.section_header[i].sh_link])->sh_offset;//sh_link gives The section header index of the associated string table.
            break;
        }
    }
    Elf32_Shdr *sectionStringsHeader = &(map_file.section_header[map_file.header->e_shstrndx]);
    char* stringTable = map_file.mapPtr + sectionStringsHeader->sh_offset;
    if(s -> debug_mode){
        printf("Symbol table '.symtab' contains %d entries:\n", size);
    }
    printf("Num:\tValue\t\tsection_index\tSection Name\tSymobl Name\n");
    for (int i = 0; i < size; i++){

        char* section_name = symbolTable[i].foo == SHN_ABS ? "ABS" :
                             symbolTable[i].st_shndx == SHN_COMMON ? "COM" :
                             symbolTable[i].st_shndx == SHN_HIRESERVE ? "HIRS":
                             symbolTable[i].st_shndx == SHN_UNDEF ? "UND":
                             (stringTable + map_file.section_header[symbolTable[i].st_shndx].sh_name);
        printf("%d:\t%08x\t%d\t\t%-10s\t%s\n",
               i,
               symbolTable[i].st_value,
               symbolTable[i].st_shndx,
               section_name,
               (char *)symbolNames+symbolTable[i].st_name);
    }
}
void RelocationTables(state * s){
    char * RelSecName = ".rel.dyn";
    Elf32_Rel* relocationTable;
    Elf32_Shdr *dynsym;
    Elf32_Shdr *dynstr;
    Elf32_Shdr* string_table_entry = map_file.mapPtr+ map_file.header->e_shoff+(map_file.header->e_shstrndx*map_file.header->e_shentsize);
    for(int i=0; i < map_file.header -> e_shnum; i++) {
        if (map_file.section_header[i].sh_type == SHT_DYNSYM) {
            dynsym = map_file.mapPtr+map_file.header->e_shoff+(i* map_file.header->e_shentsize);
        }
        if(map_file.section_header[i].sh_type == SHT_STRTAB){
            Elf32_Shdr* entry = map_file.mapPtr+ map_file.header->e_shoff+(i* map_file.header->e_shentsize);
            char* name = map_file.mapPtr + string_table_entry->sh_offset + entry->sh_name;
    ...
    int size;
    for(int i=0; i < map_file.header -> e_shnum; i++){
        if(map_file.section_header[i].sh_type == SHT_REL){
            relocationTable = (Elf32_Rel*)( map_file.mapPtr + map_file.section_header[i].sh_offset);//calculation
            size = map_file.section_header[i].sh_size / sizeof(Elf32_Rel);//size of table
            printf("Relocation section '%s' at offset %#x contains %d entries:\n",
                   RelSecName,map_file.section_header[i].sh_offset,map_file.section_header[i].sh_size/sizeof(Elf32_Rel));
            printf(" Offset\t\t  Info\t   Type\t   Sym.Value\t sym.Name\n");
            for(int i = 0; i< size; i++){
                int offset = relocationTable[i].r_offset;
                int info = relocationTable[i].r_info;
                int symbol_val = ELF32_R_SYM(info);
                int type = ELF32_R_TYPE(info);

                Elf32_Sym *symEntry = map_file.mapPtr +  dynsym->sh_offset+ symbol_val* sizeof(Elf32_Sym);
                char* symbol_name = map_file.mapPtr + dynstr->sh_offset+symEntry->st_name;
                printf("%08x\t%08x    %d\t   %08x \t %s\n",offset,info,type,symEntry->st_value,symbol_name);
            }
            if(strcmp(RelSecName,".rel.dyn") == 0)
                RelSecName = ".rel.plt";
            else{ break;}
        }
    }
