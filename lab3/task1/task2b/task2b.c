...
void kill_virus(char *fileName, int signatureOffset, int signatureSize){
    ...
    fseek(fp3, signatureOffset, SEEK_SET);
    char bits[signatureSize];
    for (int i = 0; i < signatureSize; ++i)
        bits[i] = 0x90;//NOP in ASCII
    fwrite(bits, 1, (size_t) signatureSize, fp3);
    fclose(fp3);
}
...
void fix_file (void ){
    ...
    fgetc(stdin);
    kill_virus(buff, offset, virus_size);
}
